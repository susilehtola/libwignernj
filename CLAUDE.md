...
| `include/wignernj_quadmath.hpp` | C++11 `__float128` extension (requires `BUILD_QUADMATH=ON`): explicit specialisations of every `wignernj::symbol3j<T>`/etc. template at `T = __float128`, plus a non-template `real_ylm_in_complex_ylm(int, wignernj_cfloat128_t *)` overload (`std::complex<__float128>` is not portable across standard libraries) |
