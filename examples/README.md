...
|----------|-------------------------------|--------------------------------------------|
| C        | `c/all_symbols.c`             | `#include "wigner.h"`                      |
| C++      | `cpp/all_symbols.cpp`         | `#include "wigner.hpp"`                    |
| Fortran  | `fortran/all_symbols.f90`     | `use wigner` from `libwignernj_f03`        |
| Python   | `python/all_symbols.py`       | `import wigner`                            |
...
# Python (requires pip-installed wigner package)
...
* **C++ wrapper** (`wigner::symbol3j(double, …)`) and **Fortran
...
* **Python wrapper** accepts integers, floats, or `fractions.Fraction`
  objects directly.  `wigner.wigner3j(0.5, 0.5, 1, 0.5, -0.5, 0)`
  returns `1/√6`.
